...
const Hero = () => {
    ...
    const bgDotRef = useRef<HTMLDivElement>(null);
    const strokeDotsRef = useRef<HTMLDivElement[]>([]);

useEffect(() => {
  const el = tmRef.current;
  const bgDot = bgDotRef.current;
  const strokeDots = strokeDotsRef.current;
  ...
  useEffect(() => {
    const clickTarget = clickMeRef.current;
    const heroContent = heroContentRef.current;
    const circleReveal = circleRevealRef.current;

    if (!clickTarget || !heroContent || !circleReveal) return;

    const handleClick = (e: MouseEvent) => {
      const bounds = circleReveal.getBoundingClientRect();
  ...
  return (
    <div className="hero-content_wall" ref={heroContentRef}>
      {/* Existing structure */}
            ...
            {/* ... other elements ... */}
                ...
                <div className="hero_tm">
                                            ...
                                            <div className="tm" ref={tmRef}>
                                                ...
                                                <div className="tm_dot-bg" ref={bgDotRef}></div>
                                                ...
                                                <div className="tm_dot-box stroke">
                                                    <div className="tm_dot _1 stroke" ref={(el) => (strokeDotsRef.current[0] = el!)}
                                                    ></div>
                                                    <div className="tm_dot _2 stroke" ref={(el) => (strokeDotsRef.current[1] = el!)}
                                                    ></div>
                                                </div>
                                                </div>
